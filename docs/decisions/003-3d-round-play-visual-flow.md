...
- **2026-07-15 update (spectator koozie follows the active card):** occupied seat cards reflow across the lower arc, but the static spectator cup previously used `displaySeatIndex` and the fixed eight-seat pose ring. On sparse tables that put the cup beside, rather than directly in front of, the throwing player's card. `ParkedKoozie` is presentation chrome, not a canonical pose: it now receives the active card's `seatDisplayAngle` and docks through `koozieRestPositionAtAngle`. This is a deliberate exception to the fixed pose-ring rule; live remote cup/dice frames and the roller's interactive physics dock remain unchanged. Never use this reflowed angle for pose transforms.
...
- **2026-07-15 update (eight seats + occupied-card reflow):** room capacity is fixed at
  ...
  approved, kicked, or forfeited. Pose transforms deliberately remain on a separate,
  uniform eight-seat full circle so reflow cannot change canonical dice coordinates.
